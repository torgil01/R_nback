convert_dat_csv <- function(data_dir) {
  ...
  source(here::here("summarize_trial2.R"))
  source(here::here("summarize_failed_trial.R"))
  ...
  for (i in 1:length(csv_files)) {
    ...
    if (dim(df.csv)[1] == 150) {
      ...
      df.row <- summarize_trial2(df.csv)
    } else {
      cat(csv_files[i], "is incomplete .. skipping", "\n")
      df.row <- summarize_failed_trial(df.csv)
    }
    df <- df %>% add_row(df.row)
  }

  df <- df %>% filter(id != "")
  df <- df %>% mutate(source = "LAB")
  # replace NaN with NA
